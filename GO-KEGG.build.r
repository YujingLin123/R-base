...
记录使用clusterProfiler进行GO/KEGG富集分析时，根据分析的物种来选择和准备背景数据集，包括支持的模式物种的获取，非模式物种Orgdb包的构建，通用富集分析等。

clusterProfiler相关的博客共有三篇，共同食用，效果更好 :wink: ：

博客富集分析 —— clusterProfiler
博客富集分析 —— clusterProfiler：不同物种的GO+KEGG富集分析
博客富集分析 —— clusterProfiler：Visualization
总结
...
5. references
clusterProfiler github
clusterProfiler paper
clusterProfiler book
clusterProfiler manual5.
clusterProfiler ducumentation
用AnnotationForge进行非模式物种注释构建
用AnnotationHub获取非模式物种注释信息
# gene set enrichment analysis # GSEA # enrichment analysis # over representation analysis # ORA # clusterProfiler # GO # KEGG # AnnotationHub # AnnotationForge # enricher
 基因组注释（四）：非编码RNA的注释-用Infernal软件对Rfam 12进行RNA注释
富集分析 —— clusterProfiler：Visualization 
文章目录 站点概览
1. 1. GO数据库过表达分析(ORA)的背景数据集选择
2. 2. KEGG数据库过表达分析(ORA)的背景数据集选择
3. 3. 通用富集分析 —— enricher()和GSEA()
4. 4. GO和KEGG描述查询
4.1. 4.1. GO term 查询
4.2. 4.2. KEGG Pathway 查询
5. 5. references
© 2018 – 2022  南方枝叶
